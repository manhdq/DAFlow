...
from utils.general import get_model_complexity_info
from utils.utils import AverageMeter
from torchvision import transforms, utils
from torch.utils import data

##Usage: python train_SDAFNet_viton.py -b 8 --name VITON
...
def main():
    ...
    sdafnet = SDAFNet_Tryon(ref_in_channel=6, source_in_channel=3)
    if not opt.no_cuda:
        sdafnet = sdafnet.cuda()
    macs, params = get_model_complexity_info(sdafnet,
                            input_dict={
                                'ref_input': (6,256,192),
                                'source_image': (3, 256, 192),
                                'ref_image': (3, 256, 192)
                            })
