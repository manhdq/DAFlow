...
def DAWarp(feat, offsets, att_maps, sample_k, out_ch, detach=False):
    ...
    # offsets (b*k,2,h,w)
    # att_maps (b,k,h,w)
    att_maps = torch.repeat_interleave(att_maps, out_ch, 1)  # (b,k*256*h*w)
    B,C,H,W = feat.size()
    multi_feat = torch.repeat_interleave(feat, sample_k, 0)  # (b*k,256,h,w)
    if detach:
...
class DAFlowNet(nn.Module):
    def __init__(self, num_pyramid, fpn_dim=256, head_nums=1):
        ...
        for i in range(num_pyramid):
            # self-MFE for model img 2k:flow 1k:att map
    ...
    def forward(self,
        ...
        for i in range(len(source_feats)):
            ...
            ## Post-DAWarp for source and reference imageslast_multi_cross_offsets = F.interpolate(last_multi_cross_offsets, scale_factor=2, mode='bilinear')
            if return_all:
...
class SDAFNet_Tryon(nn.Module):
    ...
    def forward(self, ref_input, source_image, ref_image, use_light_en_de=True, return_all=False, warp_feature=True):
        ...
        result = self.dafnet(source_image, ref_image, source_feats, reference_feats, 
                        use_light_en_de=use_light_en_de, return_all=return_all, warp_feature=warp_feature)
        return result